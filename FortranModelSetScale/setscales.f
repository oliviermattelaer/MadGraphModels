...
c*************************************
      ...
      alpha = 1d0/1.37d2
      print *,'alpha',alpha
c     Standard EM coupling
c      ee = dsqrt( 4d0*Pi*alpha  )
c     Mono coupling
      gm = dsqrt(Pi/alpha)
c     With Form-factor
c      gm = 0.17*dsqrt(Pi/alpha)

c     EM coupling      
c      gm = ee

c     Define constant coupling (beta = 1)
c     (for testing only)

c$$$      GAM(1)=dcmplx(gm, 0d0)
c$$$      GAM(2)=dcmplx(gm, 0d0)
...
c     Define variable coupling

      GAM(1) = dcmplx( gbeta, 0d0 )
      GAM(2) = dcmplx( gbeta, 0d0 )

      print *,'ee, gm, shat', ee, gm, shat
      print *,'betam, gbeta ', betam, gbeta
      print *,'GAM(1) ', GAM(1)
